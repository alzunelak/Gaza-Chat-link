...
class SoundEngine {
  ...
  setEngineIntensity(i) {
    ...
    this.engineGain.gain.linearRampToValueAtTime(0.001 + 0.002 * i, this.ctx.currentTime + 0.05);
    if (this.engineOsc)
      this.engineOsc.frequency.linearRampToValueAtTime(80 + i * 200, this.ctx.currentTime + 0.05);
  }
  stopEngine() {
    if (!this.engineOsc) return;
...
function spawnSmoke() {
  smokes.push({
    x: player.x + player.w / 2 - 5 + Math.random() * 6 - 3,
...
function update(dt) {
  ...
  if (Math.random() < 0.25 * (baseSpeed / 5)) spawnSmoke();
...
function endGame(win) {
  ...
  sound.stopEngine();
  if (win) { hudScore.textContent = 'YOU WIN! Score: ' + score; sound.beep(1500, 0.3); }
